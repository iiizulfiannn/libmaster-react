...
import { Table, Container, Col, Row, Button, Badge, ModalHeader, ModalBody, Form, FormGroup, Label, Input, FormText, ModalFooter, Modal } from 'reactstrap';
import { Link } from 'react-router-dom';

import { connect } from 'react-redux'
import { getAllBooksActionAdmin, getBookActionAdmin, updateBookActionAdmin, deleteBookActionAdmin, resetStateActionCreator } from '../../redux/actions/books'
...
export class Book extends Component {
  ...
  handleSubmit = async () => {
    ...
    const { id, title, image, description, genre_id, author_id, status_id, } = this.state.data;
    ...
    if (title === '') {
      ...
      try {
        ...
        this.toggleModalEdit()
        // this.notifySuccess('Success Update')
      } catch (error) {
        this.toggleModalEdit()
        // return this.notifyDanger('Failed Update')
      }
    }
  };
          ...
          <Col sm={12} md={{ size: 10, offset: 1 }}>
            <Row>
              <Col className="d-flex justify-content-between mb-2 align-items-center">
                <h1>Book Page</h1>
                <div>
                  <Button color='primary' outline size="sm" onClick={this.toggleModalAdd}>ADD BOOK</Button>
                  <ModalsAdd
            ...
            <Row>
                      ...
                      books.books.map((book, index) => {
                        return (
                          <tr key={book.id}>
                            ...
                            <td>{book.title.slice(0, 10)}...</td>
                            <td>{book.genre_name}</td>
                            <td>{book.author_name}</td>
                            <td>{book.description.slice(0, 10)}...</td>
                            <td><img src={`${URL_API}/${book.image}`} alt={`${book.title}`} style={{ width: '40px' }} /></td>
                            <td>{book.status_name}</td>
                            <td>
                              <Badge color="success" onClick={() => { this.changeIdEdit(index) }} style={{ cursor: 'pointer' }} className="mr-1">Edit</Badge>
                              <Badge color="danger" onClick={() => { this.handleDelete(book.id) }} style={{ cursor: 'pointer' }}>Delete</Badge>
                            </td>
                          </tr>
                        )
                      }) : null
              ...
              <Modal isOpen={this.state.isModalOpenEdit} toggle={this.toggleModalEdit}>
                <ModalHeader toggle={this.toggleModalEdit}>Edit Book</ModalHeader>
                <ModalBody>
                  <Form>
                    <FormGroup row>
                    ...
                    <FormGroup row>
                      ...
                      <Col sm={9}>
                        ...
                        <Input type="select" name="genre_id" value={this.state.data.genre_id} onChange={this.handleOnChangeEditBook}>
                          {
                    ...
                    <FormGroup row>
                      ...
                      <Col sm={9}>
                        ...
                        <Input type="select" name="author_id" value={this.state.data.author_id} onChange={this.handleOnChangeEditBook}>
                          {
                    ...
                    <FormGroup row>
                      ...
                      <Col sm={9}>
                        ...
                        <Input type="select" name="status_id" value={this.state.data.status_id} onChange={this.handleOnChangeEditBook}>
                          {
                    ...
                    <FormGroup row>
                      ...
                      <Col sm={9}>
                        ...
                        <Input type="textarea" name="description" value={this.state.data.description} onChange={this.handleOnChangeEditBook} />
                        {
...
const mapStateToProps = ({ books, authors, genres, status, auth }) => { //sebenarnya ini state
  return {
    books, authors, genres, status, auth
  }
}
